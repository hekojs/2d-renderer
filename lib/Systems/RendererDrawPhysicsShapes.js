...
var RendererDrawPhysicsShapes = /*#__PURE__*/function (_Heko$System) {
  _inherits(RendererDrawPhysicsShapes, _Heko$System);

  var _super = _createSuper(RendererDrawPhysicsShapes);

  function RendererDrawPhysicsShapes() {
    _classCallCheck(this, RendererDrawPhysicsShapes);

    return _super.apply(this, arguments);
  }

  _createClass(RendererDrawPhysicsShapes, [{
    key: "onAdd",
    value: function onAdd(_ref) {
      var _this = this;

      var renderer = _ref.renderer;
      this.physics = this.getWorld().plugins.get(_dPhysics["default"].Plugin);
      this.renderer = renderer;
      this.graphics = {};
      this.getWorld().events.on('added.entity', function (args) {
        return _this._createUpdateBody(args);
      });
      this.getWorld().events.on('removed.entity', function (args) {
    ...
    value: function onTick(_ref2) {
      ...
      this.queries.entities.results.forEach(function (entity) {
        if (entity.id in _this2.graphics) {
          var body = entity.getComponent(_dPhysics["default"].Components.Physics).body;
          _this2.graphics[entity.id].x = body.position.x;
          _this2.graphics[entity.id].y = body.position.y;
          _this2.graphics[entity.id].rotation = body.angle;
        }
      });
    }
  }, {
  ...
  return RendererDrawPhysicsShapes;
}(_core["default"].System);

exports["default"] = RendererDrawPhysicsShapes;
RendererDrawPhysicsShapes.queries = {
  entities: {
